...
==可以在**创建表时规定约束（通过 CREATE TABLE 语句）**，（掌握）==
...
```mysql
alter table student 
modify sname varchar(20) not null; # 成功

ALTER TABLE student
MODIFY tel CHAR(11) NOT NULL;  # 原来有NULL这样设置约束不行
```
...
INSERT INTO test2(id,  last_name, email, salary)
VALUES(1, 'Tom1', 'tom@126.com', 4600);  # 错误

INSERT INTO test2(id,  last_name, email, salary)
VALUES(2, 'Tom1', 'tom@126.com', 4600);  # 错误

INSERT INTO test2(id,  last_name, email, salary)
VALUES(2, 'Tom1', NULL, 4600);  # 可以添加NULL值

INSERT INTO test2(id,  last_name, email, salary)
VALUES(3, 'Tom2', NULL, 4700);  # 可以多次添加NULL值
...
- 当我们向主键（含AUTO_INCREMENT）的字段上添加0或NULL时，实际上会自动的往上添加指定的字段

- 开发时，一旦主键作用的字段上声明有AUTO_INCREMENT，则我们在添加数据时，就不要给主键对应的字段去赋值
...
> 面试中如果提问的话：表设计中是不是外键?
>
> mySQL提供了外键约束的功能，是为了保证数据的完整性。但是外键与级联更新适用于`单机低并发`，不适合`分布式`、`高并发集群`；级联更新是强阻塞，存在数据库`更新风暴`的风险；外键影响数据库的`插入速度`。因此在实际开发中，通过应用层面的附加逻辑，来实现外键约束的功能，确保数据的一致性。
