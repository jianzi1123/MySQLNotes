...
| FIELD(s,s1,s2,…,sn)               | 返回字符串s在字符串列表中==第一次==出现的位置                |
...
| PERIOD_ADD(time,n)           | 返回time加上n后的时间**（注意time的格式）**                  |
...
==GET_FORMAT：返回日期字符串的显示格式，函数中date_type和format_type参数取值如下：==
...
| 函数                        | 用法                                                         |
| --------------------------- | ------------------------------------------------------------ |
| PASSWORD(str)               | 返回字符串str的加密版本，41位长的字符串。加密结果`不可逆`，常用于用户的密码加密==（已经弃用）== |
| **MD5(str)**                | 返回字符串str的md5加密后的值，也是一种加密方式。若参数为NULL，则会返回NULL==（不可逆）== |
| **SHA(str)**                | 从原明文密码str计算并返回加密后的密码字符串，当参数为NULL时，返回NULL。`SHA加密算法比MD5更加安全`。==（不可逆）== |
| ENCODE(value,password_seed) | 返回使用password_seed作为加密密码加密value==（已经弃用）==   |
| DECODE(value,password_seed) | 返回使用password_seed作为加密密码解密value==（已经弃用）==   |
