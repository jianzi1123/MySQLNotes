...
- ==SELECT中出现的非组函数的字段必须声明在GRUOPBY中，包含在 GROUP BY 子句中的列不必包含在SELECT 列表中==

- ==GROUP BY声明在FROM后面，WHERE后面，ORDERBY前面，LIMIT前面==
...
- 当过滤条件中没有聚合函数时，则此过滤条件声明在WHERE或HAVING中都可以，但是建议声明在WHERE中
