...
| Gietz     | Payroll              |
+-----------+----------------------+
2889 rows in set (0.01 sec)  2889这是因为每个员工和每个部门都匹配了一遍
```
...
- 正确写法：
  ...
  # 这里忽略了department_id=null，没有被查找出来
  ```
...
# 可以给表取别名,一旦在select和where中使用，必须使用表的别名，因为执行顺序是：from->where->select
...
**UNION ALL操作符**:UNION ALL操作符返回两个查询的结果集的并集。对于两个结果集的重复部分，不去重。==（那不就是A+B）==
...
- ==`ON`：只能和JOIN一起使用，只能写关联条件。虽然关联条件可以并到WHERE中和其他条件一起写，但分开写可读性更好。==（推荐使用）
