...
- 当C大于或等于A，并且C小于或等于B时，结果为1；==闭区间[A, B]==
- 否则结果为0。
...
WHERE salary >= 6000 && salary <= 8000;  # 包含边界
...
SELECT	first_name
FROM 	employees
WHERE	first_name LIKE 'S%';  # 以S开头的
```

```mysql
SELECT last_name
FROM   employees
WHERE  last_name LIKE '_o%';  #第一个字符是别的，第二个字符是a

SELECT last_name
FROM   employees
WHERE  last_name LIKE '%o%' AND last_name LIKE '%a%';  #包含o和包含a的，写法一

SELECT last_name
FROM   employees
WHERE  last_name LIKE '%o%e%' OR last_name LIKE '%a%o%';  #包含o和包含a的，写法二
```
...
SELECT job_id
FROM   jobs
WHERE  job_id LIKE ‘IT\_%‘;  # 记住这个就行吧
```
