...
系统变量分为**全局系统变量**（需要添加`global` 关键字）以及**会话系统变量**（需要添加 `session` 关键字），有时也把全局系统变量简称为全局变量，有时也把会话系统变量称为local变量。**如果不写，默认会话级别。**静态变量（在 MySQL 服务实例运行期间它们的值不能使用 set 动态k修改）属于特殊的全局系统变量。

每一个MySQL客户机成功连接MySQL服务器后，都会产生与之对应的会话。会话期间，MySQL服务实例会在MySQL服务器内存中生成与该会话对应的会话系统变量，这些会话系统变量的初始值是全局系统变量值的复制。如下图：

![image-20211108114846634](images/image-20211108114846634.png)

- 全局系统变量针对于所有会话（连接）有效，但`不能跨重启`（重启mysql）
...
```mysql
# 修改系统变量的值：重启mysql就失效了
SET @@global.max_connections = 161;
SELECT @@global.max_connections;
SET GLOBAL max_connections = 171;  
# 只针对当前会话有效
SET @@session.character_set_client = 'gbk';
SELECT @@global.character_set_client; # utf8mb3
...
位置：只能放在 BEGIN ... END（存储过程、存储函数） 中，而且只能放在第一句
